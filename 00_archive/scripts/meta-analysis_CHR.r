...
#****************************CREATE_CHR_DATAFRAME****************************************

df_plcohort_CHR<-df_plcohort %>%
filter(population=="CHR")

View(df_plcohort_CHR)

#*****************DF_CHR_CONTINOUS_OUTCOME*****************************************
...
df_CHR_continous <- df_CHR_continous  %>%
mutate(across(c(mean_c, sd_c, n_cu, mean_nc, sd_nc, n_ncu), as.numeric))

View(df_CHR_continous)

df_CHR_continous <- escalc(measure="SMD", m1i=mean_c, sd1i=sd_c, n1i=n_cu,m2i=mean_nc, sd2i=sd_nc, n2i=n_ncu, data=df_CHR_continous)
...
df_CHR_continous  <- escalc(measure = "SMD", 
                      m1i = mean_c, sd1i = sd_c, n1i = n_cu, 
                      m2i = mean_nc, sd2i = sd_nc, n2i = n_ncu, 
                      data = df_CHR_continous , slab=paste("Study", study, "Estimate", esid)) 
                                      
                      

### assume that the effect sizes within studies are correlated with rho=0.6
V <- vcalc(vi, cluster=study, obs=esid, data=df_CHR_continous, rho=0.6)
 
### fit multilevel model using this approximate V matrix
res <- rma.mv(yi, V, random = ~ 1 | study/esid, data=df_CHR_continous , digits=3)
res

par(tck=-.01, mgp=c(1,0.01,0), mar=c(2,4,0,2))
 
dd <- c(0,diff(df_CHR_continous $study))
...
#*****************************Create_Forestplot_CHR_Continous****
...
#************************Create_Forestplot_aggregated_CHR_Continous**************************

agg <- aggregate(df_CHR_continous, cluster=study, V=vcov(res, type="obs"), addk=TRUE)
...
#**********************DF_CHR_BINARY_OUTCOME*********************


df_CHR_binary <- df_plcohort_CHR %>%
  select(studycode,
    population,
    study_type,
    `prospective?`,
...
df_CHR_binary <- df_CHR_binary %>%
  mutate_at(vars(cu_p, cu_np, ncu_p, ncu_np,or,lci_or,uci_or,p_or), as.numeric)


df_CHR_binary <- escalc(measure="OR", ai=cu_p, bi=cu_np, ci=ncu_p, di= ncu_np, data = df_CHR_binary)

View(df_CHR_binary)



df_CHR_binary<- conv.wald(out=or, ci.lb=lci_or, ci.ub=uci_or, pval=p_or, n=N_calculated, data=df_CHR_binary, transf=log)
...
#**************************fit_Meta-Analysis_Model_CHR_OR**********

### turn 'dat' into an 'escalc' object (and add study labels)
df_CHR_binary <- escalc(measure="OR", yi=yi, vi=vi, data=df_CHR_binary, slab=paste( "Study",studycode, "Estimate", esid))
 
### assume that the effect sizes within studies are correlated with rho=0.6
V <- vcalc(vi, cluster=study, obs=esid, data=df_CHR_binary, rho=0.6)
 
### fit multilevel model using this approximate V matrix
res <- rma.mv(yi, V, random = ~ 1 | study/esid, data=df_CHR_binary, digits=3)
res


#**************************Create_Forestplot_CHR_OR**************

par(tck=-.01, mgp=c(1,0.01,0), mar=c(2,4,0,2))
 
dd <- c(0,diff(df_CHR_binary$study))
rows <- (1:res$k) + cumsum(dd)
...
#***************aggregated forestplot_CHR_OR*****************

agg <- aggregate(df_CHR_binary, cluster=study, V=vcov(res, type="obs"), addk=TRUE)
